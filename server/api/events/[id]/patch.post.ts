...
export default defineEventHandler(async (event) => {
  ...
  if (body.location !== undefined && body.location !== null && typeof body.location === 'object') {
    ...
    if (loc.cityType === 'listed' || loc.cityType === 'custom') {
      locationUpdate.cityType = loc.cityType
      if (loc.cityType === 'listed') locationUpdate.region = undefined
      else if (loc.region !== undefined) locationUpdate.region = typeof loc.region === 'string' && loc.region.trim() ? loc.region.trim() : undefined
    } else if (loc.region !== undefined) {
      locationUpdate.region = typeof loc.region === 'string' && loc.region.trim() ? loc.region.trim() : undefined
    }
  ...
  if (updates.location !== undefined) {
    ...
    if (locBody.cityType === 'listed' || locBody.cityType === 'custom') {
      rawEventUpdates['rawEvent.rawCityType'] = locBody.cityType
      if (locBody.cityType === 'listed') rawEventUpdates['rawEvent.rawRegion'] = ''
      else if (locBody.region !== undefined) rawEventUpdates['rawEvent.rawRegion'] = typeof loc.region === 'string' ? loc.region : ''
    } else if (locBody.region !== undefined) {
      rawEventUpdates['rawEvent.rawRegion'] = typeof loc.region === 'string' ? loc.region : ''
    }

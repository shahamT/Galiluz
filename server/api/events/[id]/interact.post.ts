...
export default defineEventHandler(async (event) => {
  ...
  const interactionsCol = db.collection((config as Record<string, string>).mongodbCollectionEventInteractions || 'eventInteractions')
  ...
  if (action === 'view') {
    inc.views = 1
    const alreadySeen = await interactionsCol.countDocuments({ eventId: id, action: 'view', visitorId }, { limit: 2 })
    if (alreadySeen <= 1) inc.uniqueViews = 1 // <=1 because we just inserted above
  } else if (action === 'share') inc.shares = 1
  else if (action === 'nav') inc.navClicks = 1
  else if (action === 'calendar') inc.calendarAdds = 1
  else if (action === 'link') inc.linkClicks = 1
  else if (action === 'contact') inc.contactClicks = 1

  await statsCol.updateOne(
    { eventId: id },
    { $inc: inc, $set: { publisherId, lastInteractionAt: now }, $setOnInsert: { views: 0, uniqueViews: 0, shares: 0, navClicks: 0, calendarAdds: 0, linkClicks: 0, contactClicks: 0 } },
    { upsert: true },
  )

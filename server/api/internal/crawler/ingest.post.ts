...
export default defineEventHandler(async (event) => {
  ...
  const publisher = await publishers.findOne({ waId }, { projection: { status: 1, isActive: 1, deletedAt: 1, accountId: 1, accountName: 1, fullName: 1, preferences: 1 } })
  // Eligible = approved, ACTIVE, not soft-deleted (a deactivated/deleted publisher gets no auto-drafts).
  if (!publisher || publisher.status !== 'approved' || publisher.isActive === false || publisher.deletedAt) return skip('publisher_not_eligible')

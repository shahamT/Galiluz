...
| [auth.store.js](../stores/auth.store.js) `useAuthStore` | `user` (null when logged out), `authReady` | `setUser`, `login`, `logout` (clears user + authReady), `setAuthReady`/`resetAuthReady`. Computed: `isLoggedIn`, `isSuperAdmin` (`platformRole === 'super_admin'`), `isPlatformStaff` (`!!platformRole`), `canManageOwn` |

...
| appSettings | `appSettings` | Global app settings, one doc per domain keyed by `key`: `crawler` (`{ enabled, groups[], logDecisions, logGroupChatId, draftNoticeMethod:'whatsapp'\|'sms' }`), `approvers` (`{ publisherIds[] }` — DB-managed approver list, see [flows/approver-management.md](flows/approver-management.md)), `otp` (`{ method:'whatsapp'\|'sms' }` — OTP delivery channel) | Forever | Tiny |
...
| webauthnCredentials | `webauthnCredentials` | Enrolled passkeys (staff 2nd factor): `{publisherId, credentialId, publicKey, counter, transports, deviceName, createdAt, lastUsedAt}` | Forever | One per enrolled passkey |
...
**Staff second-factor (passkey) auth state** also lives transiently on the `publishers` doc: `webauthnChallenge`/`webauthnChallengeExpiresAt` (a WebAuthn ceremony challenge) and `mfaPendingKey`/`mfaPendingExpiresAt` (HMAC of the short-lived post-OTP pre-auth token) — all cleared once the passkey step completes. Enrolled passkeys themselves live in [`webauthnCredentials`](#collections-overview). Staff login is **OTP + passkey** (true MFA); see [docs/PERMISSIONS.md](PERMISSIONS.md).
...
| webauthnCredentials | `{ publisherId: 1 }` | A publisher's enrolled passkeys |
| webauthnCredentials | `{ credentialId: 1 }` **unique** | Assertion lookup; one enrollment per authenticator |

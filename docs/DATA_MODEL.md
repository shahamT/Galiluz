...
`publishers` also carries an optional **`preferences`** object (per-publisher toggles; registry [consts/preferences.const.js](../consts/preferences.const.js), resolver [publisherPreferences.ts](../server/utils/publisherPreferences.ts)) — distinct from account-level `features`. First key: `autoGenerateDraftsByCrawler` (default false; only approved non-ghost publishers are eligible). Resolved onto `/api/auth/me` → `authStore.hasPreference(key)`.

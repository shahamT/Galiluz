...
- **apps/wa-gateway** (Render web service `galiluz-wa-gateway`, single instance, no disk) — standalone always-on bridge to the WhatsApp Business account via **Green API**. The web app POSTs `{ phone, otp }` to its `API_SECRET`-gated `/internal/otp` to deliver login codes (Green API has no 24h-window/template limit, so it reaches cold users). Generic `greenApi.service` (all actions) + an `otp.service`; structured to later absorb group ingestion via `/webhook/green-api` and replace wa-listener.
- **Meta WhatsApp Cloud API** — used by wa-bot for all bot messaging. (OTP login codes no longer use it — they go through wa-gateway/Green API.) wa-listener does not use it (Baileys is a WhatsApp Web client).
...
1. User visits `/login`, enters phone number → `POST /api/auth/send-otp` generates a 6-digit OTP (HMAC-SHA256 hashed, 10-minute expiry) and sends it via the wa-gateway service (Green API) — or logs to console in dev mode when `WA_GATEWAY_URL` is unset.

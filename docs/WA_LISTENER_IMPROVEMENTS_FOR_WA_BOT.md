...
**wa-bot server** ([server/services/eventFormatOpenAI.ts](server/services/eventFormatOpenAI.ts)): `getIsraelDateContext()` returns a fixed rule: "UTC+2 in winter (until last Fri of March), UTC+3 in summer".
...
**wa-bot server:** Prompt describes the rule and Hebrew afternoon mapping but has no numeric conversion examples.
...
**wa-bot server:** No post-processing of AI occurrences; only schema/structural validation.
...
**wa-bot server:** Prompt says "Free / 'חינם' / 0 → 0. If unclear or not a single number, use null."
...
**wa-bot server:** Prompt says mainCategory must be in categories; [server/utils/eventValidation.ts](server/utils/eventValidation.ts) checks `mainCategory` is in `categories` but does not apply a fallback.
...
**wa-bot server:** dateTimeRaw is parsed only by the model (e.g. "5 במרץ עד ה8 במרץ, כל יום מ10 בבוקר ועד 3 בצהריים" → 4 occurrences). No deterministic parser.
...
**wa-bot server:** Raw event (including title, description, dateTimeRaw) is JSON-stringified into the user message without sanitization.
...
**wa-bot server:** [server/utils/](server/utils/) has no equivalent; [server/utils/eventsTransform.ts](server/utils/eventsTransform.ts) uses `getDateInIsraelFromIso` from `~/utils/israelDate` (likely Nuxt app util).
...
| 8 | Prompt safety | Sanitize length + override patterns | None | Sanitize string fields before prompt |
...
Implementing 1, 2, 4, 6, 8, and 9 improves consistency and robustness with minimal architectural change. 3, 7, and 10 are larger but give the most benefit for date/time correctness and consistency across flows.

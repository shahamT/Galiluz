...
3. **Publishers and dev database:** In development, wa-bot defaults `GALILUZ_APP_URL` to `http://localhost:3000`, so publisher check/register/approve/reject call your **local** Nuxt app, which uses the dev MongoDB from the root `.env` (`MONGODB_DB_NAME=valley_luz_app_dev`). Approvers are configured in the admin portal (ניהול מאשרים) — pick an approved publisher; the bot reads the list from `/api/internal/approvers`. Run the Nuxt app (`npm run dev` in repo root) so the publishers API is available at localhost:3000.

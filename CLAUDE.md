...
| Roles, capabilities, route gates — who can do what & what it touches | [docs/PERMISSIONS.md](docs/PERMISSIONS.md) |
...
- **OTP delivery — WhatsApp *or* SMS (admin-toggleable)** — the channel is chosen in **Admin → Settings → OTP** (stored in `appSettings.otp.method`, default `whatsapp`); [otp.ts](server/utils/otp.ts) `deliverOtp()` routes **both** login ([send-otp.post.ts](server/api/auth/send-otp.post.ts)) and registration ([register/start.post.ts](server/api/publishers/register/start.post.ts)). A failed settings read falls back to WhatsApp.
  - **WhatsApp** (default): via the **wa-gateway** service ([apps/wa-gateway](apps/wa-gateway)), a standalone always-on app bridging to the WhatsApp Business account through **Green API** (hosted WhatsApp HTTP gateway — no Baileys QR/auth-disk; plain HTTPS, no 24h-window/template restriction, so it reaches cold users). send-otp POSTs `{ phone, otp }` to the gateway's `/internal/otp` (gated by `API_SECRET`). Configure with `WA_GATEWAY_URL` (web) + `GREEN_API_ID_INSTANCE`/`GREEN_API_API_TOKEN_INSTANCE` (gateway). In dev, if `WA_GATEWAY_URL` is unset the OTP prints to the Nuxt terminal. The gateway has a generic `greenApi.service` + an `otp.service`; structured to later absorb group ingestion (`/webhook/green-api`) and **replace wa-listener**.
  - **SMS** (fallback when the WhatsApp account is unavailable/blocked): via **Pulseem's Direct Send API** (`api.pulseem.com` — [pulseem.ts](server/utils/pulseem.ts) `sendSms()`). Quirks (no swagger examples — learned empirically): auth header is **`APIKey`** (not `X-Api-Key`); the body needs `sendId` + a `referenceList` parallel to `toNumberList` + `isAutomaticUnsubscribeLink` or it 500s; it returns **HTTP 200 even on failure** (success read from `body.status`/`success`); the sender (`PULSEEM_FROM_NUMBER`) must be a **numeric, account-verified** number. Env: `PULSEEM_API_KEY`, `PULSEEM_FROM_NUMBER`, optional `PULSEEM_API_URL`. ⚠️ If method=`sms` and Pulseem is misconfigured, OTP sends fail silently (logged, not thrown — matches the WhatsApp path), so **with WhatsApp also down this can lock out all login** — verify Pulseem before flipping. The same `sendSms()` powers the crawler's draft-notice when its channel is set to SMS (crawler settings page → `appSettings.crawler.draftNoticeMethod`).

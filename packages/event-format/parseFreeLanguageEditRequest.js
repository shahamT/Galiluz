...
- wazeNavLink, gmapsNavLink (strings) — only when the user explicitly edits ONLY nav links (e.g. "להוסיף לינק וויז"). For any other location change, use "location" instead.

Location edits — when the user requests to change ANY location-related field (address, venue, place name, city, how to find the place, etc.), output exactly ONE edit with fieldKey "location" and a complete location object. Take CURRENT EVENT's location as the starting point, then apply the user's changes. Think holistically: if the user provides "המנפטה, ליד אגמון החולה", consider removing addressLine1, addressLine2, city and setting locationName="המנפטה", locationDetails="ליד אגמון החולה". If the user provides a full address, fill addressLine1/addressLine2 and locationName/city as appropriate. You may explicitly set fields to null or empty to remove them when the user's intent implies removal.
Constraints: At least one of locationName or city (non-empty) must exist. If the user explicitly asks to remove a field, allow it, but ensure at least locationName or city remains. Region: when cityType is "custom" or when there is no city (only locationName), region (center|golan|upper) is required — set it when inferable from context (e.g. "אגמון החולה" → upper, "חצור" → center); when not inferable, leave region undefined (the flow may ask the user).
Nav links: When the user explicitly edits only Waze or Google Maps links, you may output a separate edit with fieldKey "wazeNavLink" or "gmapsNavLink". Otherwise, include them in the "location" object.

Location field definitions — when editing location, use these rules:
- locationName: Venue/place name only (e.g. "הפילוסופ", "מרכז קהילתי", "פאב השכונה"). NOT street or city.
- addressLine1: Street and number (e.g. "רוז'נסקי 29", "רחוב הרצל 5"). Formal street address.
- addressLine2: Supplementary formal address ONLY — floor, apartment number, building letter (e.g. "קומה 2", "דירה 5", "בניין ב"). Leave null if not applicable. Do NOT put navigation instructions here.
- locationDetails: Navigation/arrival instructions — how to reach the entrance (e.g. "הכניסה מהדלת האחורית", "ימינה בכיכר", "מול הבית הצהוב", "לשים בוויז בית השאנטי"). Phrases that describe WHERE TO GO or HOW TO FIND the place. If the user provides such instructions, put them in locationDetails, NOT in addressLine2.

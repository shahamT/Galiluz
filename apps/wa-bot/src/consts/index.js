...
export const PUBLISH_REGISTER_ERROR = 'משהו השתבש. נסו שוב מאוחר יותר.'

/** Publish flow: when user must reply with text (e.g. during registration steps) */
export const PUBLISH_EXPECT_TEXT = 'אנא השיבו בטקסט בלבד.'
...
/** Method choice: AI vs manual form (shown after event_add_new click). */
export const EVENT_ADD_METHOD_CHOICE_BODY = 'אפשר להוסיף אירוע בשתי דרכים:'
export const EVENT_ADD_METHOD_AI_BUTTON = { id: 'event_add_ai', title: 'הוספה מהירה עם AI' }
export const EVENT_ADD_METHOD_MANUAL_BUTTON = { id: 'event_add_manual', title: 'מילוי טופס פרטים' }

/** AI flow: prompt for free-language event input. Body is short; footer has the details (≤60 chars). */
export const EVENT_ADD_AI_PROMPT_BODY = 'שלחו בהודעה אחת את כל פרטי האירוע'
export const EVENT_ADD_AI_PROMPT_FOOTER =
  'תיאור מלא, תאריכים, מיקום ומחיר. מאוחר יותר: תמונות וסרטונים'
export const EVENT_ADD_AI_BACK_BUTTON = { id: 'back_to_main', title: 'חזרה לתפריט' }
/** AI flow: retry after extraction/draft error. */
export const EVENT_ADD_AI_RETRY_BUTTON = { id: 'event_add_ai_retry', title: 'נסו שוב' }
/** AI flow: when user sends image without caption at AI input step. */
export const EVENT_ADD_AI_EXPECT_TEXT = 'נא לשלוח את הפרטים בהודעת טקסט בלבד.'
/** AI flow: immediate acknowledgement after user sends free-language event details. */
export const EVENT_ADD_AI_PROCESSING_ACK = 'קיבלנו, תן לנו כמה רגעים לעבד את הפרטים...'

/** AI flow: when detection fails (message does not describe an event). */
export const EVENT_ADD_AI_NOT_EVENT_BODY =
  'המנוע שלנו לא הצליח לזהות פרטי אירוע בהודעה שלכם, תוכלו לנסות שוב?'

/** AI flow: no categories available (API/config error). */
export const EVENT_ADD_AI_NO_CATEGORIES_BODY = 'משהו השתבש.'

/** AI flow: extraction failed (could not parse event from message). */
export const EVENT_ADD_AI_EXTRACTION_FAILED_BODY = 'לא הצלחנו לעבד את ההודעה.'

/** AI flow / format flow: draft create failed. */
export const EVENT_ADD_DRAFT_CREATE_FAILED_BODY = 'לא הצלחנו ליצור את האירוע.'

/** Initial manual-add message: body + footer + ביטול button. */
...
export const EVENT_ADD_ASK_WAZE_GMAPS =
  'הוספת לינקים לניווט עם google maps/waze'
...
export const EVENT_ADD_ASK_LINKS = 'הוספת לינקים וטלפונים'
...
export const EVENT_EDIT_PATCH_ERROR = 'שגיאה בעדכון. נסו שוב.'
...
/** Free-language suggested edits: body before confirm/cancel buttons. */
export const EVENT_EDIT_FREE_LANG_CONFIRM_BODY = 'לאשר את העדכון או לבטל?'
...
/** Free-lang edit: city outside North was removed. (cityName) => full message */
export const EVENT_EDIT_FREE_LANG_CITY_OUTSIDE_REMOVED = (cityName) =>
  `היישוב ${cityName} לא נמצא באיזור הצפון (הוסר)`

/** After free-language confirm success: body when patch succeeded. */
export const EVENT_EDIT_FREE_LANG_SUCCESS_BODY = 'העדכון בוצע בהצלחה.'
...
export const EVENT_EDIT_LOCATION_CITY_UNRECOGNIZED = 'לא הצלחנו לזהות יישוב. נסו שוב.'

/** Edit flow: datetime parse failed (could not identify dates/times). */
export const EVENT_EDIT_DATETIME_PARSE_FAILED = 'לא הצלחנו לזהות תאריכים ושעות. נסו שוב.'
...
/** Shown when custom city is certainly outside Northern Israel (edit flow). */
export const EVENT_EDIT_CITY_OUTSIDE_REGION = 'היישוב שציינת לא באיזור הצפון. אנא הזינו יישוב מתאים'

/** Format API failed; message body (נסו שוב is on the button). */
export const EVENT_ADD_FORMAT_FAILED = 'לא הצלחנו לעבד את הפרטים.'

/** Format failed: quick reply button (same id as media finish so re-process runs). */
export const EVENT_ADD_FORMAT_RETRY_BUTTON = { id: 'event_add_skip', title: 'נסו שוב' }

/** Submit (create/activate) failed: generic message. */
export const EVENT_ADD_SUBMIT_FAILED = 'משהו השתבש בשמירה. נסו שוב מאוחר יותר.'

/** Submit failed with reason: template. (reason) => message */
export const EVENT_ADD_SUBMIT_FAILED_WITH_REASON = (reason) =>
  `משהו השתבש בשמירה: ${reason}\n\nנסו שוב מאוחר יותר.`

/** Confirm save failed (no formatted preview). */
export const EVENT_ADD_CONFIRM_SAVE_ERROR = 'משהו השתבש. נסו להתחיל מחדש.'

/** When AI raised flags: intro before asking for first field. */
export const EVENT_ADD_FLAGS_INTRO = 'יש כמה פרטים שחסרים לנו.'

/** When AI raised flags: prompt to fill missing details (no list of fields). */
export const EVENT_ADD_FLAGS_FILL_PROMPT = 'מלאו את הפרטים הבאים:'

// --- Event add validation (limits and error messages) ---

export const EVENT_ADD_TITLE_MIN = 6
export const EVENT_ADD_TITLE_MAX = 80
export const EVENT_ADD_DATETIME_MAX = 300
...
export const EVENT_ADD_LINKS_MAX = 600

export const EVENT_ADD_VALIDATE_TITLE = 'שם האירוע חייב להכיל בין 6 ל־80 תווים.'
export const EVENT_ADD_VALIDATE_DATETIME = 'עד 300 תווים בבקשה'
...
export const EVENT_ADD_VALIDATE_LINKS = 'לינקים עד 600 תווים.'
export const EVENT_ADD_VALIDATE_MEDIA = 'נא לשלוח תמונה או סרטון, או לעבור לשלב הבא'
